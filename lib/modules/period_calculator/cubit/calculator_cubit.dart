...
///////////// مدة استمرار الدورة الشهرية ........1=====15 يومSelectDayNumber
  ...
  void selectDurationOfTheMenstrualCycl(int index) {
...
//how long period //27.......المدة بين الدورة والدورة التالية
  ...
  DateTimeRange ?lastPeriodRange;
  DateTimeRange ?nextPeriodRange;
  void calculator() {
    try{
      if(myService.getLastDate==null||myService.getHowLongPeriod==null||myService.getDaysNumber==null){
        ...
        nextPeriod = myService.getLastDate!.add(Duration(days: myService.getHowLongPeriod!));
        ...
        getOvulationDayList();
        getNextPeriodDates();
        debugPrint(periodDates.toList().toString());
        debugPrint(nextPeriodDates.toList().toString());
        // getFirstSafeRangDates();
        // getSecondSafeRangDates();
  ...
  List<DateTime> getPeriodDates(){
    ...
    DateTime end =myService.getLastDate!.add(Duration(days: myService.getDaysNumber!));
    final periodDatesExtracted = end.difference(start).inDays;
    periodDates = List.generate(periodDatesExtracted, (i) => DateTime(start.year, start.month, start.day + (i)));
    emit(Calculate1());
    return periodDates;
  }
  List<DateTime> getNextPeriodDates(){
    ...
    final periodDatesExtracted = end.difference(start).inDays;
    nextPeriodDates = List.generate(periodDatesExtracted, (i) => DateTime(start.year, start.month, start.day + (i)));
    emit(Calculate5());
  ...
  List<DateTime> getFirstRangDates(){
    ...
    firstRangDates = List.generate(firstRangDatesExtracted, (i) => DateTime(start.year, start.month, start.day + (i)));
    emit(Calculate2());
  ...
  List<DateTime> getSecondRangDates(){
    ...
    secondRangDates = List.generate(secondRangDatesExtracted, (i) => DateTime.parse(DateTime(start.year, start.month, start.day + (i)).toString()) );
    emit(Calculate3());

    return secondRangDates;
  }
  List<DateTime> getOvulationDayList(){
    // DateTime start = ovulationDay!;
    // DateTime end = ovulationDay!.add(Duration(days: 365));
    // final ovulationDayDatesExtracted = end.difference(start).inDays;

    // ovulationDayList = List.generate(ovulationDayDatesExtracted, (i) => DateTime.parse(DateTime(start.year, start.month, start.day + ((myService.getDaysNumber!+myService.getHowLongPeriod!)-14)).toString()) );
    ovulationDayList =[DateTime(ovulationDay!.year,ovulationDay!.month,ovulationDay!.day)];
    emit(Calculate4());

    return ovulationDayList;
  }
  List<DateTime> getFirstSafeRangDates(){
    DateTime start =myService.getLastDate!.add(Duration(days: myService.getDaysNumber!));
    DateTime end =ovulationDay!.subtract(const Duration(days: 5));
    final firstSafeRangDatesExtracted = end.difference(start).inDays;
    firstSafeRangDates = List.generate(firstSafeRangDatesExtracted, (i) => DateTime(start.year, start.month, start.day + (i)));
    // emit(Calculate2());
  ...
  List<DateTime> getSecondSafeRangDates(){
    ...
    secondSafeRangDates = List.generate(secondSafeRangDatesExtracted, (i) => DateTime(start.year, start.month, start.day + (i)));
    // emit(Calculate2());
